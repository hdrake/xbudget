...
- `examples/load_example_model_grid.py` — `load_MOM6_coarsened_diagnostics()` builds a MOM6 `xgcm.Grid` (X/Y center/outer, `areacello` metric). `examples/load_example_ecco_grid.py` — `load_ECCOV4r4_budget_diagnostics()` downloads the eleven per-collection ECCO V4r4 (2010, LLC90) files from Zenodo (DOI 10.5281/zenodo.21479854), merges the monthly-mean fluxes + month-boundary snapshots (→ `*_bounds` on the outer T coord `time_bounds`) + grid geometry + geothermal input into one budget-ready dataset, and builds the **LLC90** grid with 13-tile `face_connections`. Both download from Zenodo, cached in `data/` (gitignored; only `data/README.md` tracked).
