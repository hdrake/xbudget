...
- Docs (`docs/source/`) are Sphinx with `myst_parser` (for `.md`) + `nbsphinx` (notebooks copied in from `examples/` at build time by a hook in `conf.py`). `quickstart.md` and `conventions.md` are hand-written; `.readthedocs.yaml` sets `fail_on_warning: true`, so a broken reference fails the build. Building locally also needs `pandoc` (nbsphinx).
