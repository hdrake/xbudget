...
- Docs (`docs/source/`) are Sphinx with `myst_parser` (for `.md`) + `nbsphinx` (notebooks copied in from `examples/` at build time by a hook in `conf.py`). `quickstart.md` and `conventions.md` are hand-written. Build locally with `cd docs && make html`, or exactly as CI does: `python -m sphinx -b html -W --keep-going docs/source docs/build/html`. `-W` mirrors `.readthedocs.yaml`'s `fail_on_warning: true` — a broken cross-reference fails the build.
