...
class BudgetQuery:
    ...
    data : xgcm.Grid, xr.Dataset, or None
        ...
        ``None`` inspects a convention offline, with no dataset to check
        against. Results are then *planned* names — what the convention would
    ...
    def __init__(self, data, xbudget_dict):
        if isinstance(data, xgcm.grid.Grid):
            self._ds = data._ds
        else:
            self._ds = data  # an xr.Dataset, or None
        self.budgets = parse_budgets(xbudget_dict)
    ...
    def duplicate_names(self):
        ...
        first in the dataset. Non-empty means the convention needs a rename.
        """
