...
@pytest.mark.parametrize("path", CONVENTIONS, ids=lambda p: os.path.basename(p))
def test_all_shipped_conventions_queryable(path):
    """Every shipped convention builds a query, with no colliding output names.
    ...
    # (a convention whose sides collapse to nothing is a broken convention).
    terms = q.terms()
    assert terms
