...
RECIPES = sorted((Path(__file__).parent.parent / "recipes").glob("*.yaml"))

def test_load_all_recipe_presets():
    """Test that all recipe YAML files can be loaded successfully."""
    recipes_dir = Path(__file__).parent.parent / "recipes"
    yaml_files = sorted(recipes_dir.glob("*.yaml"))

    assert len(yaml_files) > 0, "No YAML files found in recipes directory"
...
def test_save_yaml_round_trips_shipped_recipes(path, tmp_path):
